...
const SkillItem: React.FC<SkillItemProps> = ({ title, proficiency, level, icon: Icon }) => {
  return (
    <Card className="w-full max-w-sm shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-2 hover:scale-[1.02] bg-gray-800 border-gray-700"> {/* Menyesuaikan warna latar belakang dan border kartu */}
      ...
      <CardContent>
        <div className="flex justify-between items-baseline mb-2">
          <span className="text-sm text-gray-300">PROFICIENCY LEVEL</span> {/* Mengubah text-gray-600 menjadi text-gray-300 */}
          <span className="text-lg font-bold text-white">{proficiency}% {level}</span> {/* Mengubah text-gray-900 menjadi text-white */}
        </div>
        <Progress value={proficiency} className="h-2 bg-gray-700" indicatorClassName="bg-white" /> {/* Menyesuaikan warna progress bar */}
