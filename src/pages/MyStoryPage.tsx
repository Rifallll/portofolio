...
import StoryEntry from "@/components/StoryEntry"; // Import komponen StoryEntry yang baru
...
const MyStoryPage = () => {
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      <Navbar />
      <main className="flex-grow container mx-auto px-4 py-20 max-w-screen-lg"> {/* Menyesuaikan lebar maksimum untuk tampilan esai */}
        <h1 className="text-5xl md:text-6xl font-extrabold text-foreground mb-12 text-center">My Story</h1>

        <div className="space-y-16"> {/* Menambahkan spasi vertikal antar bagian cerita */}
          {storyData.map((story, index) => (
            <StoryEntry
              key={index}
              {...story}
              isReversed={index % 2 !== 0} // Mengganti posisi gambar setiap bagian kedua
              delay={index * 0.1} // Animasi bertahap untuk setiap bagian
            />
          ))}
        </div>
      </main>
